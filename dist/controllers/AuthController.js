...
export class AuthController {
    async generateCredentials(req, res, next) {
        try {
            ...
            const appId = `pg_live_${generateString(8)}`;
    ...
    async token(req, res, next) {
        try {
            ...
            if (!user || (!user.client_secret && !user.client_secret_hash)) {
                throw new HttpError(401, 'InvalidCredentials');
            }
            const hash = crypto.createHash('sha256').update(clientSecret).digest('hex');
            const storedHash = user.client_secret_hash || (user.client_secret ? crypto.createHash('sha256').update(user.client_secret).digest('hex') : null);
            if (hash !== storedHash) {
                throw new HttpError(401, 'InvalidCredentials');
            }
            const payload = {
